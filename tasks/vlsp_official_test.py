...
def register(dir_name, model_size, delim="|"):
    ...
    seqio.TaskRegistry.add(
        "vlsp_test_{}".format(model_size),
        source=seqio.TextLineDataSource(
            split_to_filepattern={
                # HACK, need to provide.
                "train": list(iter_files(target_dir, ends_with="wceptrain.pt")),
                "validation": list(iter_files(target_dir, ends_with="wceptest.pt"))
            },
            skip_header_lines=0),
        preprocessors=[
...
task_and_dirs = [
    # LARGE
    ("large", join(dirname(realpath(__file__)), "longt5_vlsp_test_{input}-{output}".format(
