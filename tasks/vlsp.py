...
def register(dir_name, model_size, delim="|"):
    ...
    seqio.TaskRegistry.add(
        "vlsp_valid_{}".format(model_size),
        source=seqio.TextLineDataSource(
            split_to_filepattern={
                "test": list(iter_files(target_dir, ends_with="vlsp2022validation.pt"))
            },
            skip_header_lines=0),
        preprocessors=[
...
task_and_dirs = [
    ("large", join(dirname(realpath(__file__)), "longt5_vlsp_{input}-{output}".format(
